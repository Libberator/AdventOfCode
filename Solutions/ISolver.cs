...
public static class SolverFactory
{
    ...
    public static IEnumerable<ISolver> CreateAllSolvers()
    {
        ...
        foreach (var definedType in assembly.DefinedTypes)
        {
            ...
            if (Activator.CreateInstance(definedType) is ISolver solver)
                yield return solver;
        }
    }

    public static IEnumerable<ISolver> CreateSolvers(int year)
    {
        for (var day = 1; day <= 25; day++)
            if (TryCreateSolver(year, day, out var solver))
                yield return solver;
    }
